...
var CouchbaseClient = function(p_Hosts, p_Bucket, p_Password, p_Callback)
{
    if (p_Hosts.length == 0)
    ...
    this.InitConnection(p_Callback);
};
...
CouchbaseClient.prototype.OnStreamDataReceived = function(p_Data, p_Callback)
{
    if (p_Data == '\n\n\n\n')
    {
        // Stream Finalized
        this.ParseStreamData(this.TempStreamData, p_Callback);
        this.TempStreamData = '';
        return;
    }
...
CouchbaseClient.prototype.ParseStreamData = function(p_Data, p_Callback)
{
...
CouchbaseClient.prototype.InitConnection = function(p_Callback)
{
    var s_Self = this;
    this.TempStreamData = '';
    ...
    this.StreamClient = http.get('http://' + this.Bucket + ':' + this.Password + '@' + this.CurrentHost + '/pools/default/bucketsStreaming/' + this.Bucket, function(p_Response)
    {
        ...
        {
            s_Self.OnStreamDataReceived(p_Data, p_Callback);
        }).once('close', function()
            {
